...
func TestGetReportMeetings(t *testing.T) {
	...
	assert.Equal(t, listmeetings.RepMeetings[0].Duration, 6, "Duration not matching")
	assert.Equal(t, listmeetings.RepMeetings[0].ID, 12345, "ID not matching")
	assert.Equal(t, listmeetings.RepMeetings[0].ParticipantsCount, 2, "Participants Count not matching")
	assert.Equal(t, listmeetings.RepMeetings[0].TotalMinutes, 11, "TotalMinutes not matching")
	assert.Equal(t, listmeetings.RepMeetings[0].Type, 2, "Type not matching")
}
